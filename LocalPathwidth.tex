...
\begin{proof}
  ...
  remaining edges, which form a star.  In the tree, for each odd
  $i\in\{1,\ldots,2k\}$ and each node $a$ whose depth is even and such
  that $a$ is the right child of its parent, contract the entire subtree
  at rooted at $a$ and the path from $a$ to its grandparent.  It is easy
  straightforward to verify by induction on $k$ that the result of this 
  operation is $Q_k$.
\end{proof}
