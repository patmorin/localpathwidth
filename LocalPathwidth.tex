...
\comment{DW: I suggest a more descriptive title, such as ``Minor-Closed Graph Classes with Bounded Layered Pathwidth''}
...
An analogous result for pathwidth holds, where the complete binary tree is the key example (the analogue of grid graphs for treewidth). Let $T_h$ be the complete binary tree of height $h$. It is well known and easily proved that  $\pw(T_h)=\ceil{h/2}$, and every forest is a minor of some complete binary tree. \citet{RS-I} proved the following converse. 
...
\comment{ 
With a little more careful analysis in the preceding proof, the $7(k+1)$ that appears everywhere could be reduced to $6(k+1)$. Does anyone care? GJ: No. DW: no}
...
\begin{proof}
   ...
   of edges of $G$ is $O(f_0(k)n)$ \comment{We do not need $O(..)$ here, right? Example reference \citep{ReedWood16}}.  
