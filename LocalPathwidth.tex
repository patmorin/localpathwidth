...
\newcommand{\subtree}[1]{\stackrel{#1}{\wedge}}
...
Since $\pw(X_H)+1\leq w(p+1)$ and $X_H$ indexes a tree-decomposition of $G[V(H)\cup C_H]$ with width at most $w$, it follows that  $\pw(X_H) \leq (w(p+1)+1)(w+1)-1$.  
...
Let $G$ be a 2-connected graph. The \emph{SPQR-tree} $S$ of $G$ is
...
Each bag $B_a$ of $\{B_a:a\in V(T)\}$ either has size in $\{2,3\}$ (when $a$ is
generated by a P-node or an S-node) or it has size at most $w(k)$ where
$w(k)+1$ is the function in \cref{3-connected} (when $a$ is generated by
...
\begin{proof}
  ...
  in a rooted binary tree, we denote by $\subtree{a}$, the subtree rooted
  at $a$, i.e., the subtree of $T$ containing all nodes that have a
  as an ancestor, including $a$ itself.

  Recall from result of \citet{BRST-JCTB91} discussed in the introduction
  that, if $T[v]$ has pathwidth greater than $2^{2h+1}-3$ then it
  contains a subdivision $T'$ of $T_{2h}$.  Note that $T'$ does not
  immediately imply the existence of $T_h$ in $T[v]$ since two or more
  distinctive nodes of $T'$ may have been generated by the same node of $S$
  (see \figref{example}).  Label each node of $T'$ with the node of $S$
  that generated it.  Recall that each node $a$ in $S$ generates a path
  in $T$ so, if we consider any subset of nodes of $T'$ with a common
  label then this subset forms a path in $T'$.
  ...
  of nodes in $\subtree{a_1}$ is disjoint from the set of labels of
  nodes in $\subtree{a_4}$.  Applying induction on $\subtree{a_1}$
  and $\subtree{a_4}$ yields two subdivisions of $T_{h-1}$ in which
...
\begin{proof}
  ...
  to a distinctive node $\nu$. We show that $G'$ contains a path, not
  using any edge of $T'$, from $v$ to some \emph{anchor node} $\eta$
  of $P_{\mu\nu}$ with $\eta\neq\nu$.  Furthermore, except for their
  ...
  \begin{figure}
    ...
    \end{center}
    \caption{Finding a $Q_k$ minor in the proof of \cref{qk-minor}. Distinctive nodes are indicated by black disks and ahchor nodes by white circles.}
    \label{qk}
  \end{figure}
  ...
  be the path in $T'$ joing $a$ to $b$.   Thus far, we have established
  ...
  from $v$ to some \emph{anchor vertex} of $P_{ab}$ such that the
  internal vertices of $P'$ are disjoint from $V(P_{ab})\cup V(H_{b})$.
  We first construct the path $P'$ in $G'[\{c_2,c_3,c_4,c_5\}]$ and then
  ...
  since the distance between two branching nodes of $T'$ is at least
...
\comment{Let $Q^*_k$ be the graph (shown in \cref{qk}) obtained by
subdividing each edge of $T_{k+1}$ and adding a vertex $v$ adjacent to
all subdivision vertices. The proof above actually shows that, if $S[v]$
contains a subdivision of $T_{9(k+1)}$ then $G$ contains a \emph{subdivision}
of $Q^*_k$.  This is a little bit surprising, since I would have guessed
that $v$ would have to be a contracted subgraph of $G$, not a single
high-degree vertex. Is it worth writing the proof to highlight this?}
...
\begin{theorem}
   Let $G$ be a graph with no $Q_k$-minor for some fixed $k$. Then there is an $O(n)$ time algorithm to find a $(w,p)$-good tree decomposition of $G$ and an $O(1)$-layered width layered path decomposition of $G$.   
\end{theorem}

\begin{proof}
   Computing the SPQR-tree $S$ of $G$ can be done in $O(n)$ time
   \cite{X}.  Computing the (width 2) path decomposition of $H_a$ for
   each $S$-node or $P$-node $a$ is easily done of in time linear in
   the size of $H_a$.  Now, for each R-node $a$, the pathwidth of $H_a$
   is bounded.  Therefore, computing the path decomposition of $H_a$
   for an R-node $a$ can be done in time linear in the size of $H_a$
   using the algorithm of So and So. These path decompositions are all
   that is needed to construct the tree $T$ and the good tree decomposition $\{ B_a:a\in V(T)\}$

   The proof that (5) implies (4) in \secref{easy} is constructive and
   immediately gives a linear time algorithm to convert the good tree decomposition $\{ B_a:a\in V(T)\}$ into a layering $L_1,\ldots,L_x$ and a path decomposition $\{B_a:a\in V(P)\}$ such that $\{B_a:a\in V(P)\}$ has layered width at most 


   From the good-tree decomposition, we 
\end{proof}
