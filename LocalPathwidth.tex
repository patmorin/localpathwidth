...
\comment{With a little more careful analysis in the preceding proof, the $7(k+1)$ that appears everywhere could be reduced to $6(k+1)$. Does anyone care?}
...
\begin{proof}
   In the following, for each $i\in\{1,\ldots,5\}$,
   ...
   of edges of $G$ is $O(n)$.  Computing the SPQR-tree $S$ of $G$ can
   be done in $O(n)$ time and the total size of the graphs $\{H_a:a\in
   V(S)\}$ is $O(n)$ \cite{GM00}.  Computing a path decomposition of
   $H_a$ having width at most 2 for each S-node or P-node $a$ is easily
   ...
   done in $O(f_1(k)|V(H_a)|)$ time \cite{Bod93,BK91,K93,CDF96}. These
   ...
   decomposition of layered width at most $f_5(k)$. We then take $f(k)
   = \max\{f_i(k) : i\in\{1,\ldots,5\}\}$.
\end{proof}
