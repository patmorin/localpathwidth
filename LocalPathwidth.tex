...
To obtain our good tree decomposition $(B_v:v\in V(T))$ of a 2-connected
graph $G$ we start with the SPQR-tree $S$ for $G$.  For each R-node
or S-node $a$ of $S$ we take a minimum-width path decomposition
$(B_v: v\in V(P_a))$ of $H_a$.  We say that the node $a$ \emph{generates}
...
whose width depends only on $k$.  What remains is to show that, for
every $v\in V(G)$, $T[v]$ has pathwidth that depends only on $k$.
...
\begin{proof}
  ...
  Finally, by Property~3 of SPQR-trees and the fact that
...
\cite[Lemma~4]{BDDEW} proved the following:
