...
\comment{DW: I am confused by the above. What is $k$? Perhaps $k$ is meant to be $r$. When we write, ``imply that there exists exactly one node $a_i$ in $S_i$'', do we mean ``imply that for all $i\in\{1,\dots,r\}$ there exists exactly one node $a_i$ in $S_i$''?}
\end{enumerate}
...
graph $G$ we start with an SPQR-tree $S$ for $G$.  For each R-node
