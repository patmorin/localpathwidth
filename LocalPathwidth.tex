...
Using this large binary tree we then piece together a subgraph of $G$
...
\begin{proof}
  ...
  remaining edges, which form a star.  Refer to \cref{contraction}. In
  the tree, for each odd $i\in\{1,\ldots,2k\}$ and each node $a$ whose
  depth is even and such that $a$ is the right child of its parent,
  contract the entire subtree rooted at $a$ and the path from $a$ to
  its grandparent.  It is straightforward to verify by induction on $k$
  that the result of this operation is $Q_k$.
\end{proof}
...
2-connected graph $G$ has several properties:
...
It is straightforward to verify that $(B_v:v\in V(T))$ is indeed a
...
Each bag $B_a$ of $(B_v:v\in V(T))$ either has size 2 (when $a$ is
...
an R-node).  Thus, $(B_v:v\in V(T))$ is a tree decomposition of $G$
...
$S$ to be the SPQR-tree of $G$ and $(B_v:v\in V(T))$ to be a tree
...
\begin{proof}[Proof that (1) implies (5)]
   ...
   2^{14(k+1)}-3$, so that the tree decomposition $(B_v:v\in V(T))$ is
   ...
   assertion that $G$ has no $Q_k$ minor.
\end{proof}
...
From Theorems~\ref{qntnpb} and \ref{MainThm}, we obtain the following corollary:
