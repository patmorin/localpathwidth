...
%\cref{application} presents an application of \cref{MainThm} to two other graph parameters, queue number and track number.
...
\begin{proof}
  ...
  be the path in $T'$ having endpoints $a$ to $b$.   
...
\comment{ 
With a little more careful analysis in the preceding proof, the $7(k+1)$ that appears everywhere could be reduced to $6(k+1)$. Does anyone care?}

\comment{The requirement that $S[v]$ contains a subdivision of $T_{7(k+1)}$ is quite a bit stronger than necessary.  It is enough if $S[v]$ contains a subdivision of a 6-subdivision of $T_{k+1}$.  This would reduce the pathwidth dependence in the next proof from $2^{14(k+1)}-3$ down to $7\cdot 2^{2(k+1)}-3$. (Even the leading 7 could be reduced further.)}
...
to discuss algorithmic complexity, which the following theorem sketches.}
...
\begin{proof}
   ...
   of edges of $G$ is $O(f_0(k)n)$.  Computing the SPQR-tree $S$ of $G$ can
   be done in $O(f_0(k)n)$ time and the total size of the graphs $\{H_a:a\in
   V(S)\}$ is $O(f_0(k)n)$ \cite{GM00}.  Computing a path decomposition of
