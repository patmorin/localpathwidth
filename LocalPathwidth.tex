...
Let us say that a tree decomposition of a graph $G$ is \emph{$(w,p)$-good} if its width is at most $w$ and, for every $v\in V(G)$, the subtree $T[v]$
has pathwidth at most $p$.
In this section we show that if $G$ has no $Q_k$ minor
then $G$ has $(w,p)$-good tree decomposition for some $w=w(k)$ and $p=p(k)$. 
