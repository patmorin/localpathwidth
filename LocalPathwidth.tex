...
For a graph $G$, a \emph{tree decomposition} of $G$, denoted by $\{B_x:x\in V(T)\}$, is a set of subsets of $V(G)$ (called \emph{bags}) indexed by the nodes of $T$, such that:
\begin{enumerate}[(i)]
...
\citet{DMW17,DMW-FOCS13} introduced the following definition. The \emph{layered width} of a tree decomposition $\{B_x:x\in V(T)\}$ of a graph $G$ is the minimum integer $\ell$ such that, for some layering $(V_0,V_1,\dots,V_t)$ of $G$, each bag $B_x$ contains at most $\ell$ vertices in each layer $V_i$. The \emph{layered treewidth} of a graph $G$, denoted by $\tb(G)$, is the minimum layered width of a tree-decomposition of $G$. 

\citet{BDDEW} study a specialization of layered treewidth by defining the \emph{layered pathwidth} of a graph $G$ to be the minimum layered width of a path-decomposition of $G$, denoted by $\pb(G)$.\ldots 

\comment{David W.\ is probably best suited to write a summary of the relationship between \cite{BDDEW} and the current paper.}
...
In \cref{easy-section} we prove $(5)\implies (4)\implies(3)\implies(2)\implies(1)$. In \cref{hard-section} we close the loop by proving $(1)\implies (5)$. \cref{application} presents an application of \cref{MainThm} to two other graph parameters, queue number and track number.
...
In this section, we will be working with two related trees $S$ and $T$
and one graph $G$. To help the reader keep track of things we will use
variables $a$, $b$, and $c$ to refer to \emph{nodes} of $S$ and $T$
...
Dang which implies that any 3-connected $Q_k$-minor free graph $G$ has
...
introduction, if $\pw(T[v])> 2^{Y+1}-3$ for some $v\in V(G)$ then $T[v]$
contains a $T_Y$ minor.  We then show that this $T_Y$ minor in $T[v]$,
for sufficiently large $Y$, implies that $G$ has a $Q_k$ minor in which
...
To use \cref{dang-theorem} we need a small helper lemma.
...
\begin{proof}
  ...
  remaining edges, which form a star.  Refer to \cref{contraction}. In the
  tree, for each node $a$ whose depth is even and such that $a$ is the
  right child of its parent, contract the entire subtree rooted at $a$
  and the path from $a$ to its grandparent.  It is straightforward to
  verify by induction on $k$ that the result of this operation, after 
  removing parallel edges, is $Q_k$.
\end{proof}

\begin{figure}
   \begin{center}
      \includegraphics{figs/contraction}
   \end{center}
   \caption{The graph $T_{2k}^+$ contains a $Q_k$ minor. Contracting the
    the coloured edges gives the tree $T_k$}
   \label{contraction}
\end{figure}
...
\begin{proof}
  ...
  Choose $R'$ to be $T_{2k +1}$ plus a cycle on its leaves.
  ...
  a complete binary tree of height $2k$ plus an apex vertex linked to
  its leaves, which contains $Q_k$ as a minor by \cref{FindQh}.
  Now, applying \cref{dang-theorem} establishes that any 3-connected graph
  $G$ with sufficiently high pathwidth contains at least one of $P'$, $Q'$
  or $R'$ as a minor and therefore contains a $Q_k$ minor.
\end{proof}
...
\begin{proof}
  ...
  $C_i$ that contains $v$, the tree decomposition $\{B_a:a\in V(T_i)\}$ 
  has at least
  one node $a$ such that $v\in B_a$, so we make $a_v$ adjacent to exactly
  one such node for each $C_i$.  The resulting tree decomposition of $G$
...
tool formalized by \citet{dibattista.tamassia:incremental,
dibattista.tamassia:on-line, dibattista.tamassia:on-line2}.
...
Let $G$ be a 2-connected graph. The \emph{SPQR-tree} $S(G)$ for $G$ is
a tree in which each node $a\in V(S(G))$ is associated with a labelled
minor $H_a$ of $G$.  For each node $a$ of $S(G)$ each edge $xy\in E(H_a)$
...
%     the edge the $xy$ a virtual edge in each of $H_{a_1},\ldots,H_{a_k}$.
   ...
   \item Otherwise $G$ has a cutset $\{x,y\}$ such that 
     ...
     $H_a$ contains
     the real edge $xy$ if $xy\in E(G)$.  Now the edge $xy$ appears in each
     $S_i$ as a real edge in exactly one node $a_i$ of $S_i$.  To complete
     $S(G)$ we make $a$ adjacent to each of $a_1,\ldots,a_k$ and we make
     the edge the $xy$ a virtual edge in each of $H_{a_1},\ldots,H_{a_k}$.
...
Let $S$ be the SPQR-tree for a 2-connected graph $G$.
...
as $S[v]=S[\{a\in V(S(G)): v\in V(H_a)\}]$. The SQPR-tree $S$ for a 
2-connected graph $G$ has several properties that we make use of:
...
Each bag $B_a$ of $\{B_a:a\in V(T)\}$ either has size 2 (when $a$ is
...
an R-node).  Thus, $\{B_a:a\in V(T)\}$ is a tree decomposition of $G$
whose width is upper bounded by a function of $k$.  What remains is to show that, for
every $v\in V(G)$, $T[v]$ has pathwidth that is upper bounded by a function of $k$.
...
\begin{lemma}
  \label{roof-lifting}
  If $T[v]$ has pathwidth greater than $2^{2Y+1}-3$, then $S[v]$ contains
  a subdivision of $T_Y$.
\end{lemma}
...
\begin{proof}
  ...
  in a rooted binary tree, we denote by $T_a$, the subtree rooted
  ...
  corresponds to a unique node of $S$.  If we contract all nodes of
  $T''$ sharing a common label, then we obtain a subtree $T'''$ of $S[v]$
  that is a subdivision of $T_{Y}$.
\end{proof}
...
\begin{proof}
  ...
  between any pair of branching nodes in $T'$ and between any branching
  node and any leaf in $T'$ has length at least 7.

  It is convenient to work with a simplified SPQR-tree $S'$ and
  ...
  Because of Property~6 of SPQR trees, this simplification is safe 
  in the sense that, if we find a $Q_k$
  ...
  our efforts finding the $Q_k$ minor in $G'$.  Recall that $Q_k$ is
  ...
  cutset $\{v,x_i\}$ in $G$ and $vx_i$ is a virtual edge in $H_{c_i}$
  and $H_{c_{i+1}}$.  Refer to \cref{paths}.  We claim that,
  ...
  from $\bigcup_{j={i+1}}^r V(H_{c_j})\setminus\{v,x_i\}$.  By concatenating
  $P_2,\ldots,P_{r-1}$ we obtain a path $P_{ab}$ from $x_1\in V(H_a)$ to $x_{r-1}\in V(H_b)$ that we call the \emph{subdivision path} for nodes
  $a$ and $b$.
  ...
  Thus far, we have established that $G'[\{c_2,\ldots,c_{r-1}\}]$ 
  contains a simple path $P_{ab}$
  ...
  $v$ to some vertex of $P_{ab}$ such that the internal vertices of $P'$ 
  are disjoint from
  $V(P)\cup V(H_{b})$.  To prove this, there are two cases to consider:
  \begin{enumerate}
    \item $c_i$ is an R-node, for some $i\in\{2,\ldots,5\}$:  Since
    $H_{c_i}$ is 3-connected, there are three paths in $H_{c_i}$ with endpoints
    $v$ and $x_i$ and no other vertices in common.
  ...
  Finally, by Properties~2 and 4 of SPQR-trees and the fact that
  all virtual edges are incident to $v$, every branching node $a$ of 
  $T'$ is either
  an R-node or a P-node. In the case where $a$ is a P-node, all the
  subdivision paths that begin or end at a vertex of $H_a$ include the same
  vertex of $H_a$.  In the case where $a$ is an R-node, each subdivision
  path that begins or ends at a vertex of $H_a$ includes a different vertex
  (for up to 3 different vertices $x$, $y$, and $z$). Now, since $H_a$
  is 3-connected, these three vertices are in the same component of
  $H_{a}-\{v\}$. In particular, $H_a-\{v\}$ contains an edge-minimal tree that
...
\comment{With a little more careful analysis in the last paragraph of the preceding proof, the $7(k+1)$ that appears everywhere could be reduced to $6(k+1)$. Does anyone care?}
...
\begin{proof}[Proof that (1) implies (5)]
   ...
   Consider the good tree decomposition $\{B_a:a\in V(T)\}$ of $G$ described in
   ...
   2^{14(k+1)}-3$, so that this tree decomposition is
   $(w,2^{14(k+1)}-3)$-good.  Otherwise, by \cref{roof-lifting}, there
   is a vertex $v\in V(G)$ such that the SPQR-tree $S$ has a subtree
   $S[v]$ that contains a subdivision of $T_{7(k+1)}$.  Therefore,
   by \cref{qk-minor}, $G$ contains a $Q_k$ minor, contradicting the
   supposition that $G$ has no $Q_k$ minor.
\end{proof}
...
\citet[Lemma~4]{BDDEW} proved the following:

\begin{lemma}[BDDEW 2015]
\label{tnpb}
...
%\comment{Pat asks: Is the following result still worth including now that we were able to prove \cref{MainThm}? Actually, I see now that these results appear already in \citet[Lemma 5 and Theorem 4]{BDDEW}.}
%
%\begin{lemma}
%\label{log}
