...
\comment{DW: Do we want to define $\text{ltw}(G)$ and $\text{lpw}(G)$?}
...
\begin{lemma}
If a graph $G$ has a $(w,p)$-good tree decomposition, then $G$ has layered pathwidth at most $w(p+1)(w+1)$. 
\end{lemma}
...
\comment{DW: I have introduced the above two lemmas to highlight that they do not assume a minor-closed class.}
...
\begin{proof}
Let $P'$ be obtained from the complete binary tree $T_k$ by adding two dominant vertices $u_1$ and $u_2$. \comment{DW: I assume $u_1u_2$ is meant to be an edge} Let  $Q'$ be the graph obtained from the outerplanar graph $\nabla_{k}$, whose weak dual is a complete binary tree of height $k$, by adding a dominant vertex $v$.  Let $R'$ be the graph obtained from $T_{2k +1}$ by adding a cycle on its leaves, so that $R'$ is planar. 
...
\comment{DW: Do we need reference \citep{dibattista.tamassia:incremental}? I guess this reference is subsumed by \citep{dibattista.tamassia:on-line, dibattista.tamassia:on-line2}?}
...
%     the edge the $xy$ a virtual edge in each of $H_{a_1},\ldots,H_{a_k}$.
   ...
   \item Otherwise $G$ has a cutset $\{x,y\}$ such that 
     ...
     $G-\{x,y\}$.  For each $i\in \{1,\ldots,r\}$, let $\tilde{G}_i$
     be $G[V(C_i)\cup\{x,y\}]$ along with the additional edge $xy$.
     \comment{DW: Are we adding this additional edge $xy$ even if $xy$ already exists? Then we should say that $\tilde{G}_i$ is a multigraph?  Is the additional edge $xy$ considered to be real?}
     Note that (because of the inclusion of $xy$) each $\tilde{G}_i$  is
     2-connected, so each has an SPQR-tree $S_i$.  Then an SPQR-tree
     for $G$ is obtained by creating a node $a$ (a P-node) with $H_a$
     being a dipole graph with vertices $x$ and $y$ and having $k$
     ...
     $H_{a_i}$. To complete $S$ we make $a$ adjacent to each of $a_1,\ldots,a_k$ and we make
     the edge $xy$ a virtual edge in each of $H_{a_1},\ldots,H_{a_k}$.

\comment{DW: I am confused by the above. What is $k$? Perhaps $k$ is meant to be $r$. When we write, ``imply that there exists exactly one node $a_i$ in $S_i$'', do we mean ``imply that for each $i\in\{1,\dots,r\}$ there exists exactly one node $a_i$ in $S_i$''?}
\end{enumerate}
...
obvious. \comment{DW: I do not like this sentence. It suggests that the bags are already defined, but in fact the following sentences define the bags.}
...
\comment{DW: Is $S[v]$ defined? What tree decomposition indexed by $S$ are we refering to here? By $S[v]$ do we mean the set of nodes $a$ in $S$ for which the corresponding subgraph $H_a$ contains $v$? This needs to be made explicit.}
...
GJ: Yes it is surprising at first sight. Note that in the $R$-nodes we do need to forbid $Q_k$ as a minor, not enough to forbid a $Q^*_k$-subdivision. So there the minor relation is needed. }
...
\comment{DW: Do you agree with the above comment?}
...
\comment{DW: $\max\{f_i(k) : i\in\{1,\ldots,5\}\}$ should be $\max\{f_i(k) : i\in\{0,1,\ldots,5\}\}$, right?}
